...
Texture DisplayScreen::start_splash_tex;

bool DisplayScreen::init()
{
	// Load shared texture
	start_splash_tex.load_from_file(textures_path("RoyCloud.png"));

	// The position (0,0) corresponds to the center of the texture
	float wr = start_splash_tex.width * 0.5;
	float hr = start_splash_tex.height * 0.5;
...
void DisplayScreen::update(bool paused, bool start_start, bool game_over, CurrentTime game_time) {
...
void DisplayScreen::draw(const mat3& projection)
{
	...
	glBindTexture(GL_TEXTURE_2D, start_splash_tex.id);
